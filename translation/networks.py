...
class AdaINGen(nn.Module):
    ...
    def encode(self, images, training=False, flag=False):
        ...
        if flag: 
            return pre_content, share_content, domain_content, domain_mapping, style_fake
        else: 
            return share_content, domain_content, domain_mapping, style_fake
    ...
    def assign_adain_params(self, adain_params, model):
        ...
        for m in model.modules():
            if m.__class__.__name__ == "AdaptiveInstanceNorm2d":
                mean = adain_params[:, :m.num_features] #Bxnum_fx1x1
                std = adain_params[:, m.num_features:2*m.num_features] # hmm, hehe
                m.bias = mean.contiguous().view(-1) #deep copy, transfer to dim=1 => [1, Bxnum_f, 1, 1], buz apply GAP before?
...
##################################################################################
class StyleEncoder(nn.Module): #simliar to VGG architecture, fully convolutional
    def __init__(self, n_downsample, input_dim, dim, style_dim, norm, activ, pad_type):
        ...
        self.model += [nn.AdaptiveAvgPool2d(1)] # global average pooling, output dim: (B,dim,1,1)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, cmask=None):
        return self.model(x) 

    def forward_adain(self, x):
        adain_layer = nn.Sequential(*list(self.model.children())[:1]);
        return adain_layer(x);

    def forward_after_adain(self, x):
        decode_layer_2 = nn.Sequential(*list(self.model.children())[1:]); 
        return decode_layer_2(x);
...
class ResBlock(nn.Module):
    ...
    def forward(self, x):
        residual = x
        out = self.model(x) 
        out += residual # sum up residual, dimension unchanged
        return out
